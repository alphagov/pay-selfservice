...
const checkServiceNameValidation = (options) => {
  ...
  cy.get('input[name="serviceName"]').clear({ force: true })
  if (expectedInputValue) {
    cy.get('input[name="serviceName"]').type(expectedInputValue)
  }
  cy.get('input[name="serviceName"]').should('have.value', expectedInputValue)
  ...
  cy.get('input[name="serviceName"]').should('have.class', 'govuk-input--error')
  cy.get('#service-name-error').should('contain.text', expectedErrorMessage)
  cy.get('input[name="serviceName"]').should('have.value', expectedInputValue)
}

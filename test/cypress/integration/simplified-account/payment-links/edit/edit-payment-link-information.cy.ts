  ...
  describe('Admin view', () => {
    ...
    describe('Welsh payment link', () => {
      beforeEach(() => {
        ...
        cy.visit(EDIT_PAYMENT_LINK_INFO_URL(GatewayAccountType.LIVE, WELSH_PAYMENT_LINK), { failOnStatusCode: false })
      })
      ...
      it('welsh only ui functions', () => {
        ...
        cy.get('#service-content').find('form').get('#name').should('have.value', WELSH_PAYMENT_LINK.name)
        cy.get('#service-content').find('form').get('#description').should('have.value', WELSH_PAYMENT_LINK.description)
      })
    })

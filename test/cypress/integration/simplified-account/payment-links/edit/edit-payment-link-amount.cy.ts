...
const EDIT_PAYMENT_LINK_AMOUNT_URL = (serviceMode = 'test', paymentLink: Partial<ProductData>) =>
  `/service/${SERVICE_EXTERNAL_ID}/account/${serviceMode}/payment-links/${paymentLink.external_id}/edit/amount`
...
describe('Edit payment link amount', () => {
  ...
  describe('Non-admin view', () => {
    ...
    beforeEach(() => {
      ...
      cy.visit(EDIT_PAYMENT_LINK_AMOUNT_URL(GatewayAccountType.TEST, ENGLISH_PAYMENT_LINK), { failOnStatusCode: false })
    })
  ...
  describe('Admin view', () => {
    ...
    describe('English payment link', () => {
      beforeEach(() => {
        ...
        cy.visit(EDIT_PAYMENT_LINK_AMOUNT_URL(GatewayAccountType.LIVE, ENGLISH_PAYMENT_LINK))
      })

...
function patchAccountByServiceIdUpdateApplePaySuccess (serviceId, accountType, allowApplePay) {
  const path = `/v1/api/service/${serviceId}/account/${accountType}`
  return stubBuilder('PATCH', path, 200, {
    request: gatewayAccountFixtures.validUpdateToggleApplePayRequest(allowApplePay)
  })
...
function patchAccountByServiceIdUpdateGooglePaySuccess (serviceId, accountType, allowApplePay) {
  const path = `/v1/api/service/${serviceId}/account/${accountType}`
  return stubBuilder('PATCH', path, 200, {

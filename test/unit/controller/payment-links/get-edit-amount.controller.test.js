...
describe('GET edit amount controller', () => {
  ...
  it(
    'should send field values from session when rendering page for fixed amount',
    () => {
      lodash.set(req, 'session.editPaymentLinkData', {
  ...
  it(
    'should send field values from session when rendering page for variable amount',
    () => {
      lodash.set(req, 'session.editPaymentLinkData', {
        externalId: productExternalId,
        price: '',
  ...
  it(
    ...
    () => {
      ...
      expect(session).not.toHaveProperty('amountPageRecovered')
    }
  )
})

...
describe('adminusers client - create forgotten password', function () {
  ...
  describe('success', () => {
    ...
    const request = userFixtures.validForgottenPasswordCreateRequest(existingUserEmail)

    before((done) => {
      provider
        .addInteraction(
          new PactInteractionBuilder(FORGOTTEN_PASSWORD_PATH)
            .withState(`a user exists with email ${existingUserEmail}`)
    ...
    it('should create a forgotten password entry successfully', function (done) {
      adminUsersClient.users.createForgottenPassword(request.username).should.notify(done)
    })
  })
  ...
  describe('bad request', () => {
    ...
    it('should error when forgotten password creation if mandatory fields are missing', function (done) {
      adminUsersClient.users
        .createForgottenPassword(request.username)

    ...
    it('should authenticate a user successfully', function (done) {
      adminUsersClient.users
        ...
        .should.be.fulfilled.then(function (user) {
          ...
          expect(
            _.isEqual(user.serviceRoles[0].gatewayAccountIds, validUserResponse.service_roles[0].gateway_account_ids)
          ).to.be.equal(true)
      ...
      provider
        .addInteraction(
          new PactInteractionBuilder(`${USER_PATH}/authenticate`)
            ...
            .withRequestBody(request)
            .withStatusCode(401)
            .withResponseBody(pactify(unauthorizedResponse))
            .build()
        )
        .then(() => done())
    })
      ...
      provider
        .addInteraction(
          new PactInteractionBuilder(`${USER_PATH}/authenticate`)
            ...
            .withRequestBody(request)
            .withStatusCode(400)
            .withResponseBody(pactify(badAuthenticateResponse))
            .build()
        )
        .then(() => done())
    })

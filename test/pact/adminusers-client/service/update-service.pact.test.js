...
describe('adminusers client - patch request to update service', function () {
  ...
  describe('a valid update service patch request to update all fields', () => {
    ...
    it('should update service successfully', async function () {
      const service = await adminUsersClient.updateService(existingServiceExternalId, validUpdateServiceRequest)
      expect(service.externalId).to.equal(existingServiceExternalId)
      expect(service.merchantDetails).to.have.property('name', merchantDetails.name)
      expect(service.merchantDetails).to.have.property('addressLine1', merchantDetails.address_line1)
      expect(service.merchantDetails).to.have.property('addressLine2', merchantDetails.address_line2)
      expect(service.merchantDetails).to.have.property('addressCity', merchantDetails.address_city)
      expect(service.merchantDetails).to.have.property('addressCountry', merchantDetails.address_country)
      expect(service.merchantDetails).to.have.property('addressPostcode', merchantDetails.address_postcode)
      expect(service.merchantDetails).to.have.property('url', merchantDetails.url)
      expect(service.merchantDetails).to.have.property('telephoneNumber', merchantDetails.telephone_number)
      expect(service.merchantDetails).to.have.property('email', merchantDetails.email)
  ...
  describe('an invalid update service patch request', () => {
    ...
    it('should reject promise', () => {
      return adminUsersClient.updateService(existingServiceExternalId, invalidRequest).should.be.rejected
    })
  })
})

...
describe('Credentials endpoints', () => {
  describe('The ' + paths.credentials.index + ' endpoint', () => {
    afterEach(() => {
      nock.cleanAll()
      app = null
    })
    ...
    it(
      'should display empty credential values when no gateway credentials are set',
      done => {
        ...
        buildGetRequest(paths.credentials.index, app)
          .expect(200)
          .expect(response => {
            expect(response.body.currentGatewayAccount.credentials).toHaveLength(0) // eslint-disable-line
          })
          .end(done)
      }
    )

    it('should display received credentials from connector', done => {
      connectorMock.get(CONNECTOR_ACCOUNT_PATH)
        .reply(200, {
          'payment_provider': 'sandbox',
          'gateway_account_id': '1',
      ...
      buildGetRequest(paths.credentials.index, app)
        .expect(200)
        .expect(response => {
          expect(response.body.currentGatewayAccount.credentials).toEqual({ 'username': 'a-username' })
        })
        .end(done)
    })
    ...
    it('should return the account', done => {
      ...
      buildGetRequest(paths.credentials.index, app)
        .expect(200)
        .expect(response => {
          expect(response.body.currentGatewayAccount.gateway_account_id).toBe('1')
        })
        .end(done)
    })

    it('should display an error if the account does not exist', done => {
      connectorMock.get(CONNECTOR_ACCOUNT_PATH)
        .reply(404, {
    ...
    it(
      'should display an error if connector returns any other error',
      done => {
        connectorMock.get(CONNECTOR_ACCOUNT_PATH)
          .reply(999, {
    ...
    it(
      'should display an error if the connection to connector fails',
      done => {
  ...
  describe('The ' + paths.credentials.edit + ' endpoint', () => {
    afterEach(() => {
      nock.cleanAll()
      app = null
    })
    ...
    it('should display payment provider name', done => {
      connectorMock.get(CONNECTOR_ACCOUNT_PATH)
        .reply(200, {
          'payment_provider': 'sandbox',
          'gateway_account_id': '1',
      ...
      buildGetRequest(paths.credentials.edit, app)
        .expect(200)
        .expect(response => {
          expect(response.body.currentGatewayAccount.payment_provider).toBe('sandbox')
        })
        .end(done)
    })
  ...
  describe('The ' + paths.notificationCredentials.edit + ' endpoint', () => {
    afterEach(() => {
      nock.cleanAll()
      app = null
    })
  ...
  describe('The notification credentials', () => {
    afterEach(() => {
      nock.cleanAll()
      app = null
    })
    ...
    it('should pass through the notification credentials', done => {
      connectorMock.get(CONNECTOR_ACCOUNT_PATH)
        .reply(200, {
      ...
      buildGetRequest(paths.notificationCredentials.index, app)
        .expect(200)
        .expect(response => {
          expect(response.body.currentGatewayAccount.notification_credentials).toEqual({ username: 'a-notification-username' })
        })
        .end(done)
    })
  })

  describe('The provider update credentials endpoint', () => {
    afterEach(() => {
      nock.cleanAll()
      app = null
    })
    ...
    it(
      'should send new username, password and merchant_id credentials to connector',
      done => {
    ...
    it(
      'should send new username, password, merchant_id, sha_in_passphrase and sha_out_passphrase credentials to connector',
      done => {
        connectorMock.patch(CONNECTOR_ACCOUNT_CREDENTIALS_PATH, {
          'credentials': {
            'username': 'a-username',
    ...
    it(
      'should send any arbitrary credentials together with username and password to connector',
      done => {
        connectorMock.patch(CONNECTOR_ACCOUNT_CREDENTIALS_PATH, {
          'credentials': {
            'username': 'a-username',
    ...
    it('should display an error if connector returns failure', done => {
  ...
  describe('The provider update notification credentials endpoint', () => {
    let session
    afterEach(() => {
      nock.cleanAll()
      app = null
    ...
    it(
      'should send new username and password notification credentials to connector',
      done => {
    ...
    it(
      'should should flash a relevant error if no password is sent',
      done => {
        ...
        buildFormPostRequest(path, sendData, true, app)
          .expect(res => {
            ...
            expect(res.headers.location).toBe(paths.notificationCredentials.edit)
            expect(session.flash.genericError).to.have.property('length').toBe(1)
            expect(session.flash.genericError[0]).toBe('Enter a username')
          })
          .end(done)
      }
    )
      ...
      'should should flash a relevant error if no password is sent',
        ...
        buildFormPostRequest(path, sendData, true, app)
          .expect(res => {
            ...
            expect(res.headers.location).toBe(paths.notificationCredentials.edit)
            expect(session.flash.genericError).to.have.property('length').toBe(1)
            expect(session.flash.genericError[0]).toBe('Enter a password')
          })
          .end(done)
      }
    )
    ...
    it(
      'should should flash a relevant error if too short a password is sent',
      done => {
        ...
        buildFormPostRequest(path, sendData, true, app)
          .expect(res => {
            ...
            expect(res.headers.location).toBe(paths.notificationCredentials.edit)
            expect(session.flash.genericError).to.have.property('length').toBe(1)
            expect(session.flash.genericError[0]).toBe('Password must be 10 characters or more')
          })
          .end(done)
      }
    )

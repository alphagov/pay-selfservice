...
describe('create service otp resend validation', () => {
  afterEach((done) => {
    nock.cleanAll()
    app = null
    done()
  ...
  describe('post to otp resend page', () => {
    it(
      'should redirect to otp verify page on valid telephone number submission',
      done => {
    ...
    it('should ignore cookie telephone number', done => {
      ...
      supertest(app)
        ...
        .expect('Location', paths.selfCreateService.otpVerify)
        .expect(() => {
          expect(registerInviteData.telephone_number).toBe(submittedTelephoneNumber)
        })
        .end(done)
    })

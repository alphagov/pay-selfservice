...
describe('The logged in endpoint', () => {
  it('should render ok when logged in', done => {
  ...
  it('should redirect to otp login if no otp', done => {
...
describe('The postlogin endpoint', () => {
  it(
    'should redirect to root and clean mockSession of all but passport,currentGatewayAccountId  and last_url',
    done => {
...
describe('The otplogin endpoint', () => {
  afterEach(() => {
    nock.cleanAll()
  })

  it('should render and send key on first time', done => {
    const user = mockSession.getUser()
  ...
  it('should render and not send key on second time', done => {
    const user = mockSession.getUser()
...
describe('The afterOtpLogin endpoint', () => {
  it('should redirect to login to the last url', done => {
    const user = mockSession.getUser()
    const session = mockSession.getMockSession(user)
    const req = {
      session: session,
    ...
    expect(res.redirect.calledWith(lastUrl)).toBe(true)
    done()
  })

  it('should redirect to root if last url is not defined', done => {
    const user = mockSession.getUser()
    ...
    expect(req.session.secondFactor).toBe('totp')
    expect(req.session.version).toBe(1)
    done()
  })
})
...
describe('login get endpoint', () => {
  it(
    'should set the right flash message if both fields are empty',
    done => {
  ...
  it(
    'should set the right flash message if individual fields are empty',
    done => {
...
describe('login post endpoint', () => {
  ...
  it(
    'should display an error if csrf token does not exist for the login post',
    done => {
      request(getApp())
...
describe('otp login post endpoint', () => {
  it(
    'should display an error if csrf token does not exist for the login post',
    done => {
      const user = mockSession.getUser()
...
describe('otp send again post endpoint', () => {
  it(
    'should display an error if csrf token does not exist for the send again post',
    done => {
      const user = mockSession.getUser()
      const session = mockSession.getMockSession(user)
      delete session.csrfSecret
...
describe('direct login after user registration', () => {
  it(
    'should redirect user to homepage on a successful registration',
    done => {

...
describe('User clicks on Logout', function () {
  ...
  it('should redirect to login page when session.version is undefined', function (done) {
    ...
    const theMockSession = mockSession.getMockSession({sessionVersion: 2, services: ['1'], gateway_account_ids: [gatewayAccountId]})
    delete theMockSession.version

    let app = mockSession.createAppWithSession(getApp(), theMockSession)
    ...
    request(app)
      ...
      .expect(() => {
        expect(theMockSession.version).to.be.undefined // eslint-disable-line
      })
      .end(done)
  })

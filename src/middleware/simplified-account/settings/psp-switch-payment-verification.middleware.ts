...
async function canStartPspPaymentVerificationTask(req: ServiceRequest, _: ServiceResponse, next: NextFunction) {
  ...
  switch (gatewayAccount.getSwitchingCredential().paymentProvider) {
    ...
    case STRIPE:
      ...
      break
    default:
      return next(new InvalidConfigurationError(`Could not determine switching payment provider for service [service_external_id: ${service.externalId}]`))
  }
  const thisTask = tasks.findTaskById('make-a-live-payment')
  if (thisTask.status === TaskStatus.NOT_STARTED) {
    return next()
  } else {
    return next(new TaskAccessedOutOfSequenceError(

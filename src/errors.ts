...
class GatewayTimeoutError extends Error {
  ...
  constructor(message: string, gatewayName: GatewayName) {
    super(message)
    this.name = this.constructor.name
  ...
  static Ledger(message: string): GatewayTimeoutError {
    return new GatewayTimeoutError(message, GatewayName.LEDGER)
  }
}

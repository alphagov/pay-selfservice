...
async function createService(serviceName, serviceNameCy, serviceOrgType = 'central') {
  ...
  const sandboxGatewayAccount = await connectorClient.createGatewayAccount(
    'sandbox',
    'test',
    serviceName,
    null,
  ...
  const actualAccountId = stripeTestGatewayAccount
    ? stripeTestGatewayAccount.gateway_account_id
    : sandboxGatewayAccount.id

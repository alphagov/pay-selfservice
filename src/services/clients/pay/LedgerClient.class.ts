    ...
    return {
      search: async (params: TransactionSearchParams) => {
        ...
        return {
          ...
          transactions: response.data.results.map((transactionData) => Transaction.fromLedgerResponse(transactionData)),
        }
      },
      get: async (transactionExternalId: string, gatewayAccountId: number) => {
        ...
        return Transaction.fromLedgerResponse(response.data)
      },
      ...
      disputes: async (transactionExternalId: string, gatewayAccountId: number) => {
        ...
        return response.data.transactions.map((transactionData) => Transaction.fromLedgerResponse(transactionData))
      },
    }
  }

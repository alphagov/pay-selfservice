...
class WorldpayTasks {
  ...
  constructor (gatewayAccount, serviceExternalId, switchingPsp = false) {
    ...
    if (!gatewayAccount.allowMoto) {
      this.tasks.push(WorldpayTask.flexCredentialsTask(serviceExternalId, gatewayAccount.type, gatewayAccount.worldpay3dsFlex, this.findTask('worldpay-credentials')?.status === TASK_STATUS.COMPLETED, switchingPsp))
    }
...
class WorldpayTask {
   ...
   * @returns {WorldpayTask}
   */
  static flexCredentialsTask (serviceExternalId, accountType, worldpay3dsFlexCredential, ableToStart, isSwitchPspTask = false) {

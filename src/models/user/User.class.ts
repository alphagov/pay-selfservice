...
class User {
  ...
  constructor(userData: UserData) {
    ...
    this.numberOfLiveServices = this.serviceRoles.filter(
      (serviceRole) => serviceRole.service.currentGoLiveStage === 'LIVE'
    ).length
  }
  ...
  hasPermission(serviceExternalId: string, permissionName: string) {
    return lodash
      .get(this.getRoleForService(serviceExternalId), 'permissions', [])
  ...
  isAdminUserForService(serviceExternalId: string) {
    return (
      this.serviceRoles.filter(
        (serviceRole) => serviceRole.role?.name === 'admin' && serviceRole.service?.externalId === serviceExternalId
      ).length > 0
    )
  }
}

...
describe('arrange-users-by-role', () => {
  it('should arrange existing team members by role', () => {
    const adminUser = new User(
      userFixtures.validUserResponse({
        ...
        service_roles: { service: { service: { external_id: SERVICE_ID } } },
      })
    )
    ...
    expect(teamMembersRoleMap['view-only'][0])
      .to.have.property('removeLink')
      .to.equal('/service/service-id-123abc/account/test/settings/team-members/user-id-for-view-only-user/delete')
    expect(teamMembersRoleMap['view-only'][0])
      .to.have.property('changePermissionLink')
      .to.equal('/service/service-id-123abc/account/test/settings/team-members/user-id-for-view-only-user/permission')
  ...
  it('should arrange invited team members by role', () => {
    ...
    expect(invitedTeamMembersRoleMap['view-only'][0])
      .to.have.property('email')
      .to.equal('invited_view-only_user@user.gov.uk')
  })
})

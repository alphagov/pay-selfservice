...
describe('isNotCompanyNumber', () => {
  describe('UK company number validations', () => {
    ...
    it('should validate that company number of 8 digits without leading 0 is invalid', () => {
      expect(isNotCompanyNumber('12345678')).to.be.equal('Enter a valid Company registration number')
    })

    it('should validate that NI pre-partition company number without leading R0 is invalid', () => {
      expect(isNotCompanyNumber('R1234567')).to.be.equal('Enter a valid Company registration number')
    })

    it('should validate that company number of 6 digits is invalid', () => {
      expect(isNotCompanyNumber('123456')).to.be.equal('Enter a valid Company registration number')
    })

    it('should validate that company number of 7 digits is invalid (with special message)', () => {
      expect(isNotCompanyNumber('1234567')).to.be.equal('Company numbers in England and Wales have 8 digits and always start with 0')
    })

    it('should validate that company number of 9 digits is invalid', () => {
      expect(isNotCompanyNumber('012345678')).to.be.equal('Enter a valid Company registration number')
    })
    ...
    it('should validate that company number with prefix and 5 digits is invalid', () => {
      expect(isNotCompanyNumber('NI12345')).to.be.equal('Enter a valid Company registration number')
    })

    it('should validate that company number with prefix and 7 digits is invalid', () => {
      expect(isNotCompanyNumber('NC1234567')).to.be.equal('Enter a valid Company registration number')
    })

    it('should validate that company number with unrecognised prefix is invalid', () => {
      expect(isNotCompanyNumber('RC123456')).to.be.equal('Enter a valid Company registration number')
    })
  })
})

...
module.exports = async function getTransactionsForAllServices(req, res, next) {
  ...
  try {
    ...
    try {
      searchResultOutput = await transactionService.search(
    ...
    const downloadRoute = filterLiveAccounts
      ? paths.allServiceTransactions.download
      : paths.formattedPathFor(paths.allServiceTransactions.downloadStatusFilter, 'test')

...
import { penceToPounds, poundsToPence, safeConvertPoundsStringToPence } from '@utils/currency-formatter'
...
async function post(req: ServiceRequest<EditLinkAmountBody>, res: ServiceResponse) {
  ...
  const productUpdateRequest = ProductUpdateRequestBuilder.fromProduct(product)
    .setAmount({
      price: req.body.amountTypeGroup === 'fixed' ? safeConvertPoundsStringToPence(req.body.paymentAmount) ?? 0 : 0,

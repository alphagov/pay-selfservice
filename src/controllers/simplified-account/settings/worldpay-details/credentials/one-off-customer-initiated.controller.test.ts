...
const { req, res, nextRequest, nextReq, call } = new ControllerTestBuilder('@controllers/simplified-account/settings/worldpay-details/credentials/one-off-customer-initiated.controller')
  ...
  .withUrl(`/service/${SERVICE_EXTERNAL_ID}/account/${ACCOUNT_TYPE}/settings/worldpay-details/one-off-customer-initiated/${CREDENTIAL_EXTERNAL_ID}`)
  .withStubs({
  ...
  describe('get', () => {
    describe('worldpay details journey', () => {
      ...
      it('should call the findCredentialByExternalId method with the request parameter', () => {
        findCredentialByExternalIdStub.should.have.been.calledOnce
        findCredentialByExternalIdStub.should.have.been.calledWith(CREDENTIAL_EXTERNAL_ID)
      })
      ...
      it('should pass req, res and template path to the response method', () => {
        mockResponse.should.have.been.calledWith(req, res, 'simplified-account/settings/worldpay-details/one-off-customer-initiated-credentials')
      })

      it('should pass context data to the response method', () => {
        mockResponse.should.have.been.calledWith(sinon.match.any, sinon.match.any, sinon.match.any, {
          backLink: formatServiceAndAccountPathsFor(paths.simplifiedAccount.settings.worldpayDetails.index, SERVICE_EXTERNAL_ID, ACCOUNT_TYPE),
          credentials: {
            merchantCode: 'testMerchantCode',
            username: 'testUsername'
          }
    ...
    describe('switch psp journey', () => {
      ...
      it('should call the response method with the switch PSP backlink', () => {
        mockResponse.should.have.been.calledWith(sinon.match.any, sinon.match.any, sinon.match.any, {
          backLink: formatServiceAndAccountPathsFor(paths.simplifiedAccount.settings.switchPsp.switchToWorldpay.index, SERVICE_EXTERNAL_ID, ACCOUNT_TYPE),
          credentials: {
            merchantCode: 'testMerchantCode',
            username: 'testUsername'
          }

...
const { req, res, nextRequest, call } = new ControllerTestBuilder(
  '@controllers/simplified-account/settings/card-payments/default-billing-address-country.controller'
)
  .withAccount({
    type: ACCOUNT_TYPE,
...
describe('Controller: settings/card-payments/default-billing-address', () => {
  describe('get', () => {
    ...
    it('should pass req, res and template path to the response method', () => {
      ...
      expect(mockResponse.args[0][2]).to.equal(
        'simplified-account/settings/card-payments/default-billing-address-country'
      )
    })
  ...
  describe('post', () => {
    ...
    it('should update default billing address enabled', () => {
      expect(mockUpdateDefaultBillingAddressCountry.calledOnce).to.be.true
      expect(mockUpdateDefaultBillingAddressCountry.calledWith(SERVICE_EXTERNAL_ID, 'GB')).to.be.true
    })

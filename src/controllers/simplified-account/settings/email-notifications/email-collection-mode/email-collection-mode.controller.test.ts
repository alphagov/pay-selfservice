...
const { req, res, nextRequest, call } = new ControllerTestBuilder(
  '@controllers/simplified-account/settings/email-notifications/email-collection-mode/email-collection-mode.controller'
)
...
describe('Controller: settings/email-notifications/email-collection-mode', () => {
  ...
  describe('post', () => {
    ...
    it('should update the email collection mode', () => {
      ...
      expect(
        mockEmailService.setEmailCollectionModeByServiceIdAndAccountType.calledWith(
          SERVICE_ID,
          ACCOUNT_TYPE,
          'OPTIONAL'
        )

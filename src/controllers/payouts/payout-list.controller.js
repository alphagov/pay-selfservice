...
const listAllServicesPayouts = async function listAllServicesPayouts(req, res, next) {
  ...
  try {
    ...
    if (
      (filterLiveAccounts && !userPermittedAccountsSummary.gatewayAccountIds.length) ||
      (!filterLiveAccounts && !userPermittedAccountsSummary.hasTestStripeAccount)
    ) {
      return next(
    ...
    const payoutSearchResult = await payoutService.payouts(

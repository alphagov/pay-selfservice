...
module.exports = {
  ...
  buildPaymentList: function (connectorData, allCards, gatewayAccountId, filtersResult, route, serviceId) {
    ...
    connectorData.results.forEach(element => {
      ...
      if (!gatewayAccountId) {
        element.link = router.generateRoute(router.paths.transactions.serviceSwitchDetail, {
          chargeId: element.charge_id,
          serviceId: element.gateway_account_id
        })
      } else {

...
describe('arrange-users-by-role', () => {
  it('should arrange existing team members by role', () => {
    ...
    expect(teamMembersRoleMap['view-only'][0]).to.have.property('removeLink')
      .to.equal('/simplified/service/service-id-123abc/account/test/settings/team-members/user-id-for-view-only-user/delete')
    expect(teamMembersRoleMap['view-only'][0]).to.have.property('changePermissionLink')
      .to.equal('/simplified/service/service-id-123abc/account/test/settings/team-members/user-id-for-view-only-user/permission')

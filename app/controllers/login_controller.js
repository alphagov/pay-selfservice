...
module.exports.logInGet = function (req, res) {
  ...
  if (res.locals.flash.hasOwnProperty('error')) {
    switch (res.locals.flash.error[0]) {
      case 'invalid':
        setError({username: 'You must enter a valid username', password: 'You must enter a valid password'})
        break
      case 'empty_all':
        setError({username: 'You must enter a username', password: 'You must enter a password'})
        break
      case 'empty_username':

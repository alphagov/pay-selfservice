...
module.exports.bind = function (app) {
  ...
  const multerIdDocumentFile = multer({
    storage: multer.memoryStorage(),
    fileFilter: (req, file, cb) => {
      let isFileAllowed = true

      // check mime type - simple validation not actually checking the file content!
      // use https://en.wikipedia.org/wiki/Magic_number_(programming)#Magic_numbers_in_files
      // to validate correctly
      const allowedMimetypes = [
        'image/jpeg',
        'image/pjpeg', // Progressive JPEG, should we support this?
        'image/jpg', // not valid mime type, sometimes generated by old browsers? Double check this
        'image/png'
      ]
  ...
  }).single('id-document-file')
  app.post(merchantDetails.edit, xraySegmentCls, hasServices, multerIdDocumentFile, enforceUserAuthenticated, validateAndRefreshCsrf, cookieMessage, permission('merchant-details:update'), merchantDetailsCtrl.postEdit)
